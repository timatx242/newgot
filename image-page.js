...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (currentCard) {
    ...
    imageTitleElement.textContent = currentCard.getAttribute('data-title');
    downloadBtn.href = currentCard.getAttribute('data-pdf');

    // Добавляем функционал для кнопки "Поделиться"
    const shareBtn = document.getElementById('shareBtn');
    if (shareBtn) {
      shareBtn.addEventListener('click', async () => {
        const imageUrl = mainImage.src;
        const imageTitleText = imageTitleElement.textContent;
        ...
        if (navigator.share) {
          ...
          try {
            ...
            // Можно добавить сообщение пользователю об ошибке отправки
          }
        } else {
...
/* Функционал кнопки "Поделиться" удален, так как сама кнопка удалена из HTML */ 